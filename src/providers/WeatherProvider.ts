...
import { IOpenWeatherMapInfo, IWeatherstackError, IWeatherstackSucess } from "../interfaces/interfaces";
...
export async function getWeatherFromWeatherstack(city: string) {
  try {
    ...
    if (weatherInfo.hasOwnProperty('error') && weatherInfo.error.code === WEATHERSTACK_ERROR_NOT_FOUND) {
